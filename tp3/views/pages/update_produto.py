...
class UpdateProduto(Gtk.Box):
    def __init__(self, controller):
        super().__init__(orientation=Gtk.Orientation.VERTICAL, spacing=10)
        self.controller = controller
        label = Gtk.Label(label="Atualização de Produtos")
        self.name_entry = Gtk.Entry()
        self.name_entry.set_text("Nome do Produto")
        self.valor_entry = Gtk.Entry()
        self.valor_entry.set_text("Novo Valor do Produto")
    ...
    def on_button_clicked(self, button):
        ...
        message = self.controller.update_produto(nome, valor)
